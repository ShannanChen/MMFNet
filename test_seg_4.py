...
from networks.UNet.unet2_F2Net_nolocal_test import UNet
...
import torch
from load_datasets_transforms_4 import data_loader, data_transforms, infer_post_transforms
import os
import argparse
...
test_files = [
    {"image": image_name, "T1": images_T1, "T2": images_T2, "images_flair": images_flair}
        for image_name, images_T1, images_T2, images_flair, label_name in
        zip(test_samples['images'], test_samples['T1'], test_samples['T2'], test_samples['images_flair'])
]
...
# model.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(args.trained_weights).items()})

model.eval()
with torch.no_grad():
    for i, test_data in enumerate(test_loader):
        # images = test_data["image"].to(device)
        val_inputs, val_inputs_T1, val_inputs_T2, val_inputs_flair, val_labels = (test_data["image"].cuda(), test_data["T1"].cuda(), test_data["T2"].cuda(), test_data["images_flair"].cuda())
        x = torch.concat((val_inputs, val_inputs_T1, val_inputs_T2, val_inputs_flair), 1)
        roi_size = (64, 64, 64)
