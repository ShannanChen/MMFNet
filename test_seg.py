...
test_files = [
    {"image": image_name, "images_adc": images_adc, "images_flair": images_flair} for
    image_name, images_adc, images_flair, in
    zip(test_samples['images'], test_samples['images_adc'], test_samples['images_flair'])
]
...
with torch.no_grad():
    for i, test_data in enumerate(test_loader):
        ...
        x, x_adc, x_flair = (test_data["image"].cuda(), test_data["images_adc"].cuda(), test_data["images_flair"].cuda())
