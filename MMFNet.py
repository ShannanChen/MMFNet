...
from networks.UXNet_3D.network_backbone_F2Net import UXNET
from monai.networks.nets import UNETR, AttentionUnet
from networks.SwinUNETR.swin_unetr import SwinUNETR
from networks.UNet.unet2_F2Net_nolocal import UNet
...
train_files = [
    {"image": image_name, "images_adc": images_adc, "images_flair": images_flair, "label": label_name}
    for image_name, images_adc, images_flair, label_name in
    zip(train_samples['images'], train_samples['images_adc'], train_samples['images_flair'], train_samples['labels'])
]

val_files = [
    {"image": image_name, "images_adc": images_adc, "images_flair": images_flair, "label": label_name}
    for image_name, images_adc, images_flair, label_name in
    zip(valid_samples['images'], valid_samples['images_adc'], valid_samples['images_flair'], valid_samples['labels'])
]
...
if args.network == '3DUXNET':
...
elif args.network == 'Factorizer':
    model = ft.SegmentationFactorizer(
        in_channels=1,
        out_channels=out_classes,
        ...
        nmf=(
            ...
            {
                "tensorize": (ft.SWMatricize, {"head_dim": 8, "patch_size": 4}),
                "act": nn.ReLU,
                "factorize": ft.NMF,
                "rank": 1,
                "num_iters": 5,
    ...
    model = SegResNet(spatial_dims=3, init_filters=8, in_channels=3, out_channels=out_classes, dropout_prob=None, act=('RELU', {'inplace': True}), norm=('GROUP', {'num_groups': 8}), norm_name='', num_groups=8, use_conv_final=True, blocks_down=(1, 2, 2, 4), blocks_up=(1, 1, 1)).to(device)
...
def validation(epoch_iterator_val):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_val):
            val_inputs, val_inputs_adc, val_inputs_flair, val_labels = (
            batch["image"].cuda(), batch["images_adc"].cuda(), batch["images_flair"].cuda(), batch["label"].cuda())
            # val_outputs = model(val_inputs)
            zero_tensor_x_dwi = torch.zeros_like(val_inputs)
            zero_tensor_x_adc = torch.zeros_like(val_inputs_adc)
            zero_tensor_x_flair = torch.zeros_like(val_inputs_flair)
            val_inputs = torch.concat((val_inputs, zero_tensor_x_adc, zero_tensor_x_flair), 1)
            val_outputs_fusion, val_outputs_dwi, val_outputs_adc, val_outputs_flair = sliding_window_inference(val_inputs, (64, 64, 64), 2, model)
...
def train(global_step, train_loader, dice_val_best, hd_val_best, global_step_best):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        zero_tensor_x_dwi = torch.zeros_like(x)
        zero_tensor_x_adc = torch.zeros_like(x_adc)
        zero_tensor_x_flair = torch.zeros_like(x_flair)
        x = torch.concat((x, zero_tensor_x_adc, zero_tensor_x_flair), 1)
        logit_map_fusion, logit_map_dwi,  logit_map_adc, logit_map_flair = model(x)
        loss1 = loss_function(logit_map_dwi, y)
        loss2 = loss_function(logit_map_adc, y)
        loss3 = loss_function(logit_map_flair, y)
        loss4 = loss_function(logit_map_fusion, y)
...
HausdorffDistance = HausdorffDistanceMetric(include_background=True, reduction='mean', percentile=95, get_not_nans=False)
global_step = 0
dice_val_best = 0.0
hd_val_best = 0.0
global_step_best = 0
