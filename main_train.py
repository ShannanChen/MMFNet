...
if args.network == '3DUXNET':
    model = UXNET(
        ...
        depths=[2, 2, 2, 2],
        feat_size=[48, 96, 192, 384],  # 64, 64, 128, 256, 512, 64
        drop_path_rate=0,
    ...
    model = UNet(n_channels=3, n_classes=out_classes, width_multiplier=1, trilinear=True, use_ds_conv=False).to(device)
...
def validation(epoch_iterator_val):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_val):
            ...
            val_inputs = torch.concat((val_inputs, val_inputs_adc, val_inputs_flair), 1)
...
def train(global_step, train_loader, dice_val_best, hd_val_best, global_step_best):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        x = torch.concat((x, x_adc, x_flair), 1)
