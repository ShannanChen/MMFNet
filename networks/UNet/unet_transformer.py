...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width_multiplier=1, trilinear=True, use_ds_conv=False):
        ...
        self.fusion5 = EnhancedFeature(self.channels[4] // factor, is_first=False)

        self.fusion_layer1 = Fusion_layer(self.channels[4] // factor)
        self.fusion_layer2 = Fusion_layer(self.channels[3] // factor)
        self.fusion_layer3 = Fusion_layer(self.channels[2] // factor)
        self.fusion_layer4 = Fusion_layer(self.channels[1] // factor)
    ...
    def forward(self, x):
        ...
        enc1_dwi = self.encoder1(x1_fusion)
        x2_fusion = self.down1_fusion(enc1_dwi)
        x2_fusion = self.fusion2(x2_fusion, x2_dwi, x2_adc, x2_flair)
        enc2_dwi = self.encoder2(x2_fusion)
        x3_fusion = self.down2_fusion(enc2_dwi)
        x3_fusion = self.fusion3(x3_fusion, x3_dwi, x3_adc, x3_flair)
        enc3_dwi = self.encoder3(x3_fusion)
        x4_fusion = self.down3_fusion(enc3_dwi)
        x4_fusion = self.fusion4(x4_fusion, x4_dwi, x4_adc, x4_flair)
        enc4_dwi = self.encoder4(x4_fusion)
        x5_fusion = self.down4_fusion(enc4_dwi)
        x5_fusion = self.fusion5(x5_fusion, x5_dwi, x5_adc, x5_flair)
        enc5_dwi = self.encoder5(x5_fusion)

        x4_concat = torch.concat((enc4_dwi, x4_dwi, x4_adc, x4_flair), 1)
        x3_concat = torch.concat((enc3_dwi, x3_dwi, x3_adc, x3_flair), 1)
        x2_concat = torch.concat((enc2_dwi, x2_dwi, x2_adc, x2_flair), 1)
        x1_concat = torch.concat((enc1_dwi, x1_dwi, x1_adc, x1_flair), 1)

        # x4_fusion = self.fusion_layer1(x4_fusion, x4_dwi, x4_adc, x4_flair)
        x = self.up1_fusion(enc5_dwi, x4_concat)
        # x3_fusion = self.fusion_layer2(x3_fusion, x3_dwi, x3_adc, x3_flair)
        x = self.up2_fusion(x, x3_concat)
        # x2_fusion = self.fusion_layer3(x2_fusion, x2_dwi, x2_adc, x2_flair)
        x = self.up3_fusion(x, x2_concat)
        # x1_fusion = self.fusion_layer4(x1_fusion, x1_dwi, x1_adc, x1_flair)
...
class Fusion_layer(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv3d(dim * 3, dim, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm3d(dim),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        x4_0 = x1 * x4
        x4_0 = torch.cat((x4_0, x1), dim=1)
        x4_0 = self.conv5(x4_0)

        x = torch.cat((x1_0, x2_0, x4_0), dim=1)
        x = self.conv3(x)
