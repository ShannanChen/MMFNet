...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.Multi_swinViT = Multi_SwinTransformer(
            in_chans=in_channels,
            embed_dim=feature_size,
            window_size=window_size,
            patch_size=patch_size,
...
class Multi_SwinTransformer(nn.Module):
    """
    Swin Transformer based on: "Liu et al.,
    Swin Transformer: Hierarchical Vision Transformer using Shifted Windows
    <https://arxiv.org/abs/2103.14030>"
    ...
    def __init__(
        ...
        self.bn0 = torch.nn.BatchNorm3d(embed_dim)
        self.relu0 = torch.nn.ReLU()

        self.bn1 = torch.nn.BatchNorm3d(embed_dim * 2 ** 1)
        self.relu1 = torch.nn.ReLU()

        self.bn2 = torch.nn.BatchNorm3d(embed_dim * 2 ** 2)
        self.relu2 = torch.nn.ReLU()

        self.bn3 = torch.nn.BatchNorm3d(embed_dim * 2 ** 3)
        self.relu3 = torch.nn.ReLU()

        self.bn4 = torch.nn.BatchNorm3d(embed_dim * 2 ** 4)
    ...
    def forward(self, x_dwi, x_adc, x_flair, normalize=True):
        x0_dwi = self.patch_embed(x_dwi)
        x0_flair = self.patch_embed(x_flair)
        x0_adc = self.patch_embed(x_adc)

        x0_dwi = self.pos_drop(x0_dwi)
        x0_flair = self.pos_drop(x0_flair)
        x0_adc = self.pos_drop(x0_adc)

        x0_multi = self.relu0(self.bn0(x0_dwi * x0_flair * x0_adc))
        x0_out_dwi = self.proj_out(x0_multi + x0_dwi, normalize)
        x0_out_adc = self.proj_out(x0_multi + x0_adc, normalize)
        x0_out_flair = self.proj_out(x0_multi + x0_flair, normalize)
        x1_dwi = self.layers1[0]((x0_multi + x0_dwi).contiguous())
        x1_adc = self.layers1[0]((x0_multi + x0_adc).contiguous())
        x1_flair = self.layers1[0]((x0_multi + x0_flair).contiguous())

        x1_multi = self.relu1(self.bn1(x1_dwi * x1_flair * x1_adc))
        x1_out_dwi = self.proj_out(x1_multi + x1_dwi, normalize)
        x1_out_adc = self.proj_out(x1_multi + x1_adc, normalize)
        x1_out_flair = self.proj_out(x1_multi + x1_flair, normalize)
        x2_dwi = self.layers2[0]((x1_multi + x1_dwi).contiguous())
        x2_adc = self.layers2[0]((x1_multi + x1_adc).contiguous())
        x2_flair = self.layers2[0]((x1_multi + x1_flair).contiguous())

        x2_multi = self.relu2(self.bn2(x2_dwi * x2_flair * x2_adc))
        x2_out_dwi = self.proj_out(x2_multi + x2_dwi, normalize)
        x2_out_adc = self.proj_out(x2_multi + x2_adc, normalize)
        x2_out_flair = self.proj_out(x2_multi + x2_flair, normalize)
        x3_dwi = self.layers3[0]((x2_multi + x2_dwi).contiguous())
        x3_adc = self.layers3[0]((x2_multi + x2_adc).contiguous())
        x3_flair = self.layers3[0]((x2_multi + x2_flair).contiguous())

        x3_multi = self.relu3(self.bn3(x3_dwi * x3_flair * x3_adc))
        x3_out_dwi = self.proj_out(x3_multi + x3_dwi, normalize)
        x3_out_adc = self.proj_out(x3_multi + x3_adc, normalize)
        x3_out_flair = self.proj_out(x3_multi + x3_flair, normalize)
        x4_dwi = self.layers4[0]((x3_multi + x3_dwi).contiguous())
        x4_adc = self.layers4[0]((x3_multi + x3_adc).contiguous())
        x4_flair = self.layers4[0]((x3_multi + x3_flair).contiguous())

        x4_multi = self.relu4(self.bn4(x4_dwi * x4_flair * x4_adc))
        x4_out_dwi = self.proj_out(x4_multi + x4_dwi, normalize)
        x4_out_adc = self.proj_out(x4_multi + x4_adc, normalize)
        x4_out_flair = self.proj_out(x4_multi + x4_flair, normalize)


        return [x0_out_dwi, x1_out_dwi, x2_out_dwi, x3_out_dwi, x4_out_dwi,
                x0_out_adc, x1_out_adc, x2_out_adc, x3_out_adc, x4_out_adc,
                x0_out_flair, x1_out_flair, x2_out_flair, x3_out_flair, x4_out_flair]
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width_multiplier=1, trilinear=True, use_ds_conv=False,
        ...
        self.swinViT_dwi = SwinTransformer(
            in_chans=1,
            embed_dim=feature_size,
            window_size=window_size,
            patch_size=patch_size,
        ...
        self.swinViT_adc = SwinTransformer(
            in_chans=1,
            embed_dim=feature_size,
            window_size=window_size,
            patch_size=patch_size,
        ...
        self.swinViT_flair = SwinTransformer(
            in_chans=1,
            embed_dim=feature_size,
            window_size=window_size,
            patch_size=patch_size,
    ...
    def forward(self, x):
        ...
        flair = x[:, 2, :]
        size = x.shape[2:]
        hidden_states_out_dwi = self.Multi_swinViT(x[:, 0, :].resize(dwi.shape[0], 1, dwi.shape[1], dwi.shape[2], dwi.shape[3]),
                                                   x[:, 1, :].resize(dwi.shape[0], 1, dwi.shape[1], dwi.shape[2], dwi.shape[3]),
                                                   x[:, 2, :].resize(dwi.shape[0], 1, dwi.shape[1], dwi.shape[2], dwi.shape[3]), self.normalize)
        # hidden_states_out_adc = self.swinViT_adc(x[:, 1, :].resize(dwi.shape[0], 1, dwi.shape[1], dwi.shape[2], dwi.shape[3]), self.normalize)
        # hidden_states_out_flair = self.swinViT_flair(x[:, 2, :].resize(dwi.shape[0], 1, dwi.shape[1], dwi.shape[2], dwi.shape[3]), self.normalize)
        ...
        x3_adc = self.down2_adc(hidden_states_out_dwi[5] + x2_adc)
        x3_dwi = self.down2_dwi(hidden_states_out_dwi[0]+ x2_dwi)
        x3_flair = self.down2_flair(hidden_states_out_dwi[10]+ x2_flair)

        x4_adc = self.down3_adc(hidden_states_out_dwi[6]+ x3_adc)
        x4_dwi = self.down3_dwi(hidden_states_out_dwi[1]+ x3_dwi)
        x4_flair = self.down3_flair(hidden_states_out_dwi[11]+ x3_flair)

        x5_adc = self.down4_adc(hidden_states_out_dwi[7]+ x4_adc)
        x5_dwi = self.down4_dwi(hidden_states_out_dwi[2]+ x4_dwi)
        x5_flair = self.down4_flair(hidden_states_out_dwi[12]+ x4_flair)

        x5_concat = torch.concat((x5_dwi, x5_adc, x5_flair), 1)

        x3_1 = self.up1(x5_concat, x4_dwi, x4_adc, x4_flair)
        x2_2 = self.up2(x3_1, x3_dwi, x3_adc, x3_flair)
        x1_3 = self.up3(x2_2, x2_dwi, x2_adc, x2_flair)
        x0_4 = self.up4(x1_3, x1_dwi, x1_adc, x1_flair)
