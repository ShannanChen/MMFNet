...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width_multiplier=1, trilinear=True, use_ds_conv=False):
        ...
        self.up1 = Up(self.channels[4] * 2, self.channels[3] * 2 // factor, trilinear)
        self.up2 = Up(self.channels[3] * 2, self.channels[2] * 2 // factor, trilinear)
        self.up3 = Up(self.channels[2] * 2, self.channels[1] * 2 // factor, trilinear)
        self.up4 = Up(self.channels[1] * 2, self.channels[0] * 2, trilinear)
        self.outc = OutConv(self.channels[0] * 2, n_classes)
    ...
    def forward(self, x):
        ...
        x5_concat = torch.concat((x5_adc, x5_flair), 1)

        x = self.up1(x5_concat, x4_adc, x4_flair)
        x = self.up2(x, x3_adc, x3_flair)
        x = self.up3(x, x2_adc, x2_flair)
        x = self.up4(x, x1_adc, x1_flair)
        logits = self.outc(x)
        return logits
