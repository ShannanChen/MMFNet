...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width_multiplier=1, trilinear=True, use_ds_conv=False):
        ...
        self.up1 = Up(self.channels[4] * 2, self.channels[3] // factor, trilinear)
    ...
    def forward(self, x):
        ...
        # x4_concat = torch.concat((x4_dwi, x4_adc, x4_flair), 1)
        # x3_concat = torch.concat((x3_dwi, x3_adc, x3_flair), 1)
        # x2_concat = torch.concat((x2_dwi, x2_adc, x2_flair), 1)
        # x1_concat = torch.concat((x1_dwi, x1_adc, x1_flair), 1)

        # x5_concat = x5_dwi * x5_adc * x5_flair
        # x4_concat = x4_dwi * x4_adc * x4_flair
        # x3_concat = x3_dwi * x3_adc * x3_flair
        # x2_concat = x2_dwi * x2_adc * x2_flair
        # x1_concat = x1_dwi * x1_adc * x1_flair

        # x5_concat = np.stack([x5_dwi.cpu(), x5_adc.cpu(), x5_flair.cpu()], axis=0)
        # stacked_modalities = np.mean(x5_concat, axis=0)
        # averaged_result = (stacked_modalities > 0.5).astype(np.float32)
        # x5_concat = torch.tensor(averaged_result).cuda()
        #
        # x4_concat = np.stack([x4_dwi.cpu(), x4_adc.cpu(), x4_flair.cpu()], axis=0)
        # stacked_modalities = np.mean(x4_concat, axis=0)
        # averaged_result = (stacked_modalities > 0.5).astype(np.float32)
        # x4_concat = torch.tensor(averaged_result).cuda()
        #
        # x3_concat = np.stack([x3_dwi.cpu(), x3_adc.cpu(), x3_flair.cpu()], axis=0)
        # stacked_modalities = np.mean(x3_concat, axis=0)
        # averaged_result = (stacked_modalities > 0.5).astype(np.float32)
        # x3_concat = torch.tensor(averaged_result).cuda()
        #
        # x2_concat = np.stack([x2_dwi.cpu(), x2_adc.cpu(), x2_flair.cpu()], axis=0)
        # stacked_modalities = np.mean(x2_concat, axis=0)
        # averaged_result = (stacked_modalities > 0.5).astype(np.float32)
        # x2_concat = torch.tensor(averaged_result).cuda()
        #
        # x1_concat = np.stack([x1_dwi.cpu(), x1_adc.cpu(), x1_flair.cpu()], axis=0)
        # stacked_modalities = np.mean(x1_concat, axis=0)
        # averaged_result = (stacked_modalities > 0.5).astype(np.float32)
        # x1_concat = torch.tensor(averaged_result).cuda()

        x = self.up1(x5_concat, x4_dwi)
        x = self.up2(x, x3_dwi)
        x = self.up3(x, x2_dwi)
        x = self.up4(x, x1_dwi)
        logits = self.outc(x)
...
if __name__ == '__main__':
        input_tensor = torch.rand(1, 3, 64, 64, 64).cuda()
        # ideal_out = torch.rand(1, 2, 32, 32, 32)
        model = UNet(n_channels=3, n_classes=2, width_multiplier=1, trilinear=True, use_ds_conv=False)
        from thop import profile
        flops, params = profile(model.cuda(), (input_tensor,))
