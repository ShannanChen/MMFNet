...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x5_concat = torch.concat((x5_dwi, x5_flair), 1)

        x = self.up1(x5_concat, x4_dwi, x4_flair)
        x = self.up2(x, x3_dwi, x3_flair)
        x = self.up3(x, x2_dwi, x2_flair)
        x = self.up4(x, x1_dwi, x1_flair)
        logits = self.outc(x)
        return logits
