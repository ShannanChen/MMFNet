...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width_multiplier=1, trilinear=True, use_ds_conv=False):
        ...
        self.inc_T1 = DoubleConv(1, self.channels[0], conv_type=self.convtype)
        ...
        self.down1_T1 = Down(self.channels[0], self.channels[1], conv_type=self.convtype)
        ...
        self.down2_T1 = Down(self.channels[1], self.channels[2], conv_type=self.convtype)
        ...
        self.down3_T1 = Down(self.channels[2], self.channels[3], conv_type=self.convtype)
        ...
        self.down4_T1 = Down(self.channels[3], self.channels[4] // factor, conv_type=self.convtype)
        ...
        self.up1_T1 = Up(self.channels[4], self.channels[3] // factor, trilinear)
        self.up2_T1 = Up(self.channels[3], self.channels[2] // factor, trilinear)
        self.up3_T1 = Up(self.channels[2], self.channels[1] // factor, trilinear)
        self.up4_T1 = Up(self.channels[1], self.channels[0], trilinear)
        self.outc_T1 = OutConv(self.channels[0], n_classes)

        self.up1_fusion = Up(768, self.channels[3] // factor, trilinear)
        self.up2_fusion = Up(384, self.channels[2] // factor, trilinear)
        self.up3_fusion = Up(192, self.channels[1] // factor, trilinear)
        self.up4_fusion = Up(96, self.channels[0], trilinear)
        self.outc_fusion = OutConv(self.channels[0], n_classes)

        self.fusion1 = EnhancedFeature_nonlocal(self.channels[0], is_first=True)
        self.fusion2 = EnhancedFeature_nonlocal(self.channels[1], is_first=False)
        self.fusion3 = EnhancedFeature_nonlocal(self.channels[2], is_first=False)
    ...
    def forward(self, x):
        ...
        x = self.up1_T1(x5_T1, x4_T1)
        x = self.up2_T1(x, x3_T1)
        x = self.up3_T1(x, x2_T1)
        x = self.up4_T1(x, x1_T1)
        logits_T1 = self.outc_T1(x)

        x1_fusion = self.fusion1(0, x1_dwi, x1_adc, x1_flair, x1_T1)
        x2_fusion = self.down1_fusion(x1_fusion)
        x2_fusion = self.fusion2(x2_fusion, x2_dwi, x2_adc, x2_flair, x2_T1)
        x3_fusion = self.down2_fusion(x2_fusion)
        x3_fusion = self.fusion3(x3_fusion, x3_dwi, x3_adc, x3_flair, x3_T1)
        x4_fusion = self.down3_fusion(x3_fusion)
        x4_fusion = self.fusion4(x4_fusion, x4_dwi, x4_adc, x4_flair, x4_T1)
        x5_fusion = self.down4_fusion(x4_fusion)

        x4_concat = torch.concat((x4_fusion, x4_dwi, x4_adc, x4_flair, x4_T1), 1)
        x3_concat = torch.concat((x3_fusion, x3_dwi, x3_adc, x3_flair, x3_T1), 1)
        x2_concat = torch.concat((x2_fusion, x2_dwi, x2_adc, x2_flair, x2_T1), 1)
        x1_concat = torch.concat((x1_fusion, x1_dwi, x1_adc, x1_flair, x1_T1), 1)
        ...
        return logits_fusion, logits_dwi, logits_adc, logits_flair, logits_T1
...
class EnhancedFeature_nonlocal(nn.Module):
    def __init__(self, in_chan, is_first=False):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv3d(in_channels=5 * in_chan, out_channels=in_chan, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm3d(in_chan),
            nn.ReLU(inplace=True),
        )
        ...
        self.non_local_channel = _NonLocalBlockND_channel(4 * in_chan, inter_channels=None, dimension=3, sub_sample=False, bn_layer=True)

        self.non_local_spitial = _NonLocalBlockND_spitail(in_chan, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True)


    def forward(self, x0, x1, x2, x3, x4):
        w = torch.sigmoid(self.conv1(self.non_local_channel(torch.cat((x1, x2, x3, x4), dim=1))))
        ...
        feat_x2 = torch.mul(x2, w)
        feat_x3 = torch.mul(x3, w)
        feat_x4 = torch.mul(x4, w)
        x = self.conv3(torch.cat((self.conv2(feat_x1 + feat_x2 + feat_x3 + feat_x4), x1, x2, x3, x4), dim=1))
        if not self.is_first:
            x = self.conv(torch.cat((x0, x), dim=1))
        return x
