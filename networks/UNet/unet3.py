...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        enc1_dwi = self.encoder1_dwi(x1_dwi)
        # enc1_adc = self.encoder1_adc(x1_adc)
        # enc1_flair = self.encoder1_flair(x1_flair)
        ...
        enc5_dwi = self.encoder5_dwi(x5_dwi)
        # enc5_adc = self.encoder5_adc(x5_adc)
        # enc5_flair = self.encoder5_flair(x5_flair)

        # x5_concat = torch.concat((enc5_dwi, enc5_adc, enc5_flair), 1)
        ...
        x = self.up1(enc5_dwi, enc4_dwi)
        x = self.up2(x, enc3_dwi)
        x = self.up3(x, enc2_dwi)
        x = self.up4(x, enc1_dwi)
        logits = self.outc(x)
        return logits
