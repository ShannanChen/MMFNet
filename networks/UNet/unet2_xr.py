...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x1_adc = self.inc_relu(self.inc_bn(x1_dwi * x1_adc))
        # x1_flair = self.inc_relu(self.inc_bn(x1_dwi * x1_flair))
        # x1_dwi = x1_dwi + x1_flair

        x2_dwi = self.down1_dwi(x1_dwi)
        x2_adc = self.down1_adc(x1_adc)
        x2_flair = self.down1_flair(x1_flair)

        # x2_adc = self.relu1(self.bn1(x2_dwi * x2_adc))
        # x2_flair = self.relu1(self.bn1(x2_dwi * x2_flair))
        # x2_dwi = x2_dwi + x2_flair

        x3_adc = self.down2_adc(x2_adc)
        x3_dwi = self.down2_dwi(x2_dwi)
        x3_flair = self.down2_flair(x2_flair)

        # x3_adc = self.relu2(self.bn2(x3_dwi * x3_adc))
        # x3_flair = self.relu2(self.bn2(x3_dwi * x3_flair))
        # x3_dwi = x3_dwi + x3_flair

        x4_adc = self.down3_adc(x3_adc)
        x4_dwi = self.down3_dwi(x3_dwi)
        x4_flair = self.down3_flair(x3_flair)

        # x4_adc = self.relu3(self.bn3(x4_dwi * x4_adc))
        # x4_flair = self.relu3(self.bn3(x4_dwi * x4_flair))
        # x4_dwi = x4_dwi + x4_flair

        x5_adc = self.down4_adc(x4_adc)
        x5_dwi = self.down4_dwi(x4_dwi)
        x5_flair = self.down4_flair(x4_flair)

        # x5_adc = self.relu4(self.bn4(x5_dwi * x5_adc))
        # x5_flair = self.relu4(self.bn4(x5_dwi * x5_flair))
        # x5_dwi = x5_dwi + x5_flair

        # x5_concat = torch.concat((x5_dwi, x5_adc, x5_flair), 1)
        x5_add = x5_dwi + x5_adc + x5_flair
        x4_add = x4_dwi + x4_adc + x4_flair
        x3_add = x3_dwi + x3_adc + x3_flair
        x2_add = x2_dwi + x2_adc + x2_flair
        x1_add = x1_dwi + x1_adc + x1_flair

        x = self.up1(x5_add, x4_add)
        x = self.up2(x, x3_add)
        x = self.up3(x, x2_add)
        x = self.up4(x, x1_add)
        logits = self.outc(x)
