...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, width_multiplier=1, trilinear=True, use_ds_conv=False):
        ...
        self.up1_fusion = Up(1024, self.channels[3] // factor, trilinear)
    ...
    def forward(self, x):
        ...
        x_dwi1 = self.up1_dwi(x5_dwi, x4_dwi)
        x_dwi2 = self.up2_dwi(x_dwi1, x3_dwi)
        x_dwi3 = self.up3_dwi(x_dwi2, x2_dwi)
        x_dwi4 = self.up4_dwi(x_dwi3, x1_dwi)
        logits_dwi = self.outc_dwi(x_dwi4)

        x_adc1 = self.up1_adc(x5_adc, x4_adc)
        x_adc2 = self.up2_adc(x_adc1, x3_adc)
        x_adc3 = self.up3_adc(x_adc2, x2_adc)
        x_adc4 = self.up4_adc(x_adc3, x1_adc)
        logits_adc = self.outc_adc(x_adc4)

        x_flair1 = self.up1_flair(x5_flair, x4_flair)
        x_flair2 = self.up2_flair(x_flair1, x3_flair)
        x_flair3 = self.up3_flair(x_flair2, x2_flair)
        x_flair4 = self.up4_flair(x_flair3, x1_flair)
        logits_flair = self.outc_flair(x_flair4)
        ...
        x5_concat = torch.concat((x5_fusion, x5_dwi, x5_adc, x5_flair), 1)
        ...
        x = self.up1_fusion(x5_concat, x4_concat)
