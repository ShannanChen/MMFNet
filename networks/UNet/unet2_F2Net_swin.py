...
class EnhancedFeature_nonlocal(nn.Module):
    def __init__(self, in_chan, is_first=False):
        ...
        self.non_local_spitial = _NonLocalBlockND_spitail(3 * in_chan, inter_channels=None, dimension=3, sub_sample=True, bn_layer=True)


    def forward(self, x0, x1, x2, x3):
        # w = torch.sigmoid(self.conv1(torch.cat((x1, x2, x3), dim=1)))
        w1 = self.non_local_channel(torch.cat((x1, x2, x3), dim=1))
        w2 = self.non_local_spitial(torch.cat((x1, x2, x3), dim=1))
        w = self.conv1(w1 + w2)
        feat_x1 = torch.mul(x1, w)
        feat_x2 = torch.mul(x2, w)
        feat_x3 = torch.mul(x3, w)
