...
def data_loader(args):
    ...
    if args.mode == 'train':
        ...
        train_img_adc = sorted(glob.glob(os.path.join(root_dir, 'imagesTr_adc', '*.nii.gz')))
        ...
        train_samples['images_adc'] = train_img_adc
        ...
        valid_img_adc = sorted(glob.glob(os.path.join(root_dir, 'imagesVal_adc', '*.nii.gz')))
        ...
        valid_samples['images_adc'] = valid_img_adc
        ...
        test_img_adc = sorted(glob.glob(os.path.join(root_dir, 'imagesTs_adc', '*.nii.gz')))
        test_img_flair = sorted(glob.glob(os.path.join(root_dir, 'imagesTs_flair', '*.nii.gz')))
        test_samples['images'] = test_img
        test_samples['images_adc'] = test_img_adc
    ...
    if dataset == 'feta':
        if args.mode == 'train':
            ...
            train_transforms = Compose(
                [
                    LoadImaged(keys=["image", "images_adc", "images_flair", "label"]),
                    AddChanneld(keys=["image", "images_adc", "images_flair", "label"]),
                    Orientationd(keys=["image", "images_adc", "images_flair", "label"], axcodes="RAS"),
                    # ScaleIntensityRanged(
                    #     keys=["image", "images_adc", "images_flair"], a_min=-10, a_max=10,
                    #     b_min=0.0, b_max=1.0, clip=True,
                    # ),
                    # Resized(keys=['image', 'label'], spatial_size=(112, 112, 112)),
                    CropForegroundd(keys=["image", "images_adc", "images_flair", "label"], source_key="image"),
                    # ResizeWithPadOrCropd(keys=["image", "label"], spatial_size=(64, 64, 16)),
                    # RandCropByPosNegLabeld(
                    #     keys=["image", "label"],
                    #     label_key="label",
                    ...
                    # ),
                    RandSpatialCropSamplesd(
                        keys=["image", "images_adc", "images_flair", "label"],
                        roi_size=(64, 64, 64),
                        num_samples=crop_samples,
                        random_size=False,
                    ),
                    RandShiftIntensityd(
                        keys=["image", "images_adc", "images_flair"],
                        offsets=0.10,
                        prob=0.50,
                    ),
                    RandAffined(
                    ...
                    ToTensord(keys=["image", "images_adc", "images_flair", "label"]),
                ]
            )
            ...
            val_transforms = Compose(
                [
                    ...
                    Orientationd(keys=["image", "images_adc", "images_flair", "label"], axcodes="RAS"),
                    # ScaleIntensityRanged(
                    #     keys=["image", "images_adc", "images_flair"], a_min=0, a_max=1000,
                    #     b_min=0.0, b_max=1.0, clip=True,
                    # ),
                    CropForegroundd(keys=["image", "images_adc", "images_flair", "label"], source_key="image"),
                    ToTensord(keys=["image", "images_adc", "images_flair", "label"]),
            ...
            test_transforms = Compose(
                [
                    LoadImaged(keys=["image", "images_adc", "images_flair"]),
                    AddChanneld(keys=["image", "images_adc", "images_flair"]),
                    Orientationd(keys=["image", "images_adc", "images_flair"], axcodes="RAS"),
                    # ScaleIntensityRanged(
                    #     keys=["image", "images_adc", "images_flair"], a_min=0, a_max=1000,
                    #     b_min=0.0, b_max=1.0, clip=True,
                    # ),
                    CropForegroundd(keys=["image", "images_adc", "images_flair"], source_key="image"),
                    ToTensord(keys=["image", "images_adc", "images_flair"]),
                ]
            )
